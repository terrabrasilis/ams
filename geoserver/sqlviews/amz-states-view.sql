...
SELECT 
	su.suid, su."NM_ESTADO" AS name, su.geometry, ri.classname, ri.date, COALESCE(ri.perc, 0) AS percentage, COALESCE(ri.total, 0) AS area
FROM 
...
LEFT JOIN (
	...
	FROM 
		public."amz_states_risk_indicators" 
	WHERE
		classname = '%classname%' 
		AND
		date > '%enddate%'::date
