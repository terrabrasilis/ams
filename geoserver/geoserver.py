...
geo.create_featurestore(store_name=store_name, workspace=workspace, 
	db=store_name, host='localhost', 
	pg_user='postgres', pg_password='postgres')

for spatial_unit in spatial_units:
	geo.publish_featurestore(workspace=workspace, store_name=store_name, 
		pg_table=spatial_unit)
	geo.publish_featurestore(workspace=workspace, store_name=store_name, 
		pg_table=f'{spatial_unit}_risk_indicators')
	...
	sql = 'SELECT'
	geo.publish_featurestore_sqlview(store_name=store_name, 
		name=f'{spatial_unit}_view', sql=sql, workspace=workspace, 
